...
struct MultiLayerPerceptron : NeuralNetwork<InputType, OutputType> {
	...
	MultiLayerPerceptron(size_t n_inputs, size_t n_outputs) {
...
public:
	...
	void set_epoch(unsigned int epoch = 0) {
	...
	double fit(std::vector<TrainingInstance<InputType, OutputType>> samples) {
		...
		if (sample_size > 0) {
			...
			std::for_each(std::execution::par_unseq, samples.begin(), samples.end(), [this, &parallel_index, &samples, &outputs, &output_derivatives](auto &unused) {
			...
			parallel_index.store(0);
			std::for_each(std::execution::par_unseq, samples.begin(), samples.end(), [this, &output_layer, &parallel_index, &samples, &outputs, &sample_errors](auto &unused) {
				int i = parallel_index.fetch_add(1);
